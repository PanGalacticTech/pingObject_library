...
void pingObject::pingLoop() {
  ...
  if (pingSequencer == 0) {                                         // state 0 - Armed Mode: function waits for delay timer to finish, then triggers the sequence by 
    pingObject::triggerPing(SENSOR_SAMPLE_DELAY_MS);                   //incrementing pingSequencer
  }
...
pingObject::timeEcho();                                             // Returns pulseDuration as global variable
 ...
 pingObject::pingCalc(pulseDuration)                                 // Sub routine resets pingSequencer to 0, and puts the program back in armed mode 
  }
...
int32_t pingObject::pingCalc(uint32_t echoDuration) {
  ...
  pingObject::printFiltered_data(dataLib.recursiveFilter(cm));
...
void pingObject::outputPinControl(bool input) {
  ...
  void pingObject::printDistance_cm(uint32_t centimeters) {
  ...
  void pingObject::printFiltered_data(int16_t input) {
